...
exports.openAIStreamAPIstreamReducer = exports.openAIStreamReducer = void 0;
exports.makeOpenAIChatCompletionsOp = makeOpenAIChatCompletionsOp;
exports.makeOpenAIImagesGenerateOp = makeOpenAIImagesGenerateOp;
exports.summarizer = summarizer;
exports.makeOpenAIResponsesCreateProxy = makeOpenAIResponsesCreateProxy;
...
function makeOpenAIChatCompletionsOp(originalCreate, name) {
    function wrapped(...args) {
        const [originalParams] = args;
    ...
    const options = {
        ...
        streamReducer: exports.openAIStreamReducer,
    };
    return (0, op_1.op)(wrapped, options);
}
function makeOpenAIImagesGenerateOp(originalGenerate) {
        ...
        if (result.data) {
            result.data = await Promise.all(result.data.map(async (item) => {
                if (item.b64_json) {
                    ...
                    return (0, media_1.weaveImage)({ data: buffer, imageType: 'png' });
                }
                return item;
            }));
        }
...
exports.openAIStreamAPIstreamReducer = {
    initialStateFn: () => ({
        responses: [],
        _outputStaging: [],
    }),
            ...
            case 'response.content_part.done':
                ...
                if (state.responses.length > 0) {
                    ...
                    if (lastResponse.output && lastResponse.output[chunk.output_index]) {
                        const outputItem = lastResponse.output[chunk.output_index];
                        if (outputItem.content && outputItem.content[chunk.content_index]) {
                            Object.assign(outputItem.content[chunk.content_index], chunk.part);
                        }
                    }
                }
                break;
    ...
    finalizeFn: state => {
        if (state._outputStaging &&
            ...
            const lastOutputItem = lastResponse.output[lastResponse.output.length - 1];
            if (!lastOutputItem.content || lastOutputItem.content.length === 0) {
                lastOutputItem.content = [{}];
            }
            const lastOutputItemContent = lastOutputItem.content[lastOutputItem.content.length - 1];
...
function makeOpenAIResponsesCreateProxy(originalCreate) {
    return new Proxy(originalCreate, {
        apply: (target, thisArg, args) => {
            ...
            const weaveOpOptions = {
                ...
                ...(isStream ? { streamReducer: exports.openAIStreamAPIstreamReducer } : null),
                parameterNames: 'useParam0Object',
            };
...
function cjsPatchOpenAI(exports) {
...
function esmPatchOpenAI(exports) {
    const OpenAIProxy = commonProxy(exports);
